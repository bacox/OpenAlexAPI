...
from .author import Author, DehydratedAuthor
from .concept import Concept, DehydratedConcept
from .venue import Venue, DehydratedVenue, HostVenue
from .institution import Institution, DehydratedInstitution
...
class OpenAlex(BaseModel):
    ...
    _works_urls: dict = {
            ...
            Work: _base_url+"works?filter=title.search:"
        }
    ...
    def search_entities(self, query: str, entity_type= Union["Author","Concept","Institution","Venue","Work"], limit: int = None) -> Union[List[Author],List[Concept],List[Institution],List[Venue],List[Work]]:
        ...
        while cursor:
            ...
            # url = f"{self._base_url}{etype.__name__.lower()}s?filter=title.search:{query}&per_page={per_page}&cursor={cursor}"
            response = requests.get(url, headers=self._headers)
